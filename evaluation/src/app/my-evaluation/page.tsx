...
/**
 * The signed-in employee's own evaluation + manager dialogue. Same experience as
 * the magic-link page, but authenticated (resolved by the user↔employee link) so
 * the employee reaches it from inside the portal instead of an email link.
 */
export default function MyEvaluationPage() {
  ...
  async function respond(decision: "ACKNOWLEDGE" | "OBJECT") {
    ...
    setError(null);
    try {
      const res = await fetch(withBase("/api/my-evaluation/respond"), {
        method: "POST",
    ...
    >
      <div className="mx-auto max-w-3xl space-y-6">
        <header className="text-center">
          ...
          />
          <h1 className="text-2xl font-bold text-[#082b45]">تقييمي</h1>
          <p className="mt-1 text-sm text-[#42647d]">مراجعة تقييمك ومراسلة مديرك</p>
        </header>
        ...
        {!loading && none && (
          <div className="rounded-xl border border-[#d6e8f5] bg-white/85 p-8 text-center text-[#42647d]">
            لا يوجد لديك تقييم حتى الآن. عند إرسال مديرك لتقييمك سيظهر هنا.
          </div>
        )}

        {!loading && error && !data && !none && (
