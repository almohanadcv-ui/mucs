...
# ── 0) تحديث الكود (git pull) ─────────────────────────────────────────
# داخل السكربت عمدًا: تشغيله بدون سحب كان يبني الكود القديم بصمت (يظهر كأن
# «لا شيء تغيّر»)، وهو ما يفسّر أكثر حالات «نشرت لكن لم يتغيّر شيء».
step "0/3  تحديث الكود (git pull)"
if git -C "$REPO_DIR" pull --ff-only; then
  ok "الكود مُحدّث إلى أحدث نسخة"
else
  fail "فشل git pull — أوقفت النشر حتى لا يُبنى كود قديم. عالج التعارض ثم أعد التشغيل."
  exit 1
fi
